...
<script>
    $(function() {
        $('[name=area_id]').change(function() {
            ...
            $.ajax({
                ...
                $.each(shops, function(index, value) {
                    ...
                    $(card).addClass('area_search');
                })
                $('.area_search').css('display', 'block');
                $('.card').not('.area_search').css('display', 'none');
            }).fail(function(res) {
                alert('データ取得できませんでした。');
            })
        })
        ...
        $('[name=genre_id]').change(function() {
            $('div').removeClass('genre_search');
            areaId = $('[name="genre_id"]').val();
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                var shops = res.shops;
                $.each(shops, function(index, value) {
                    var genre_id = value.id;
                    var card = document.getElementById(genre_id);
                    $(card).addClass('genre_search');
                })
                $('.genre_search').css('display', 'block');
                $('.card').not('.genre_search').css('display', 'none');
            }).fail(function(res) {
                alert('データ取得できませんでした。');
            })
        })
