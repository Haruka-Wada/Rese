...
<div class="header__search">
    ...
    <select name="genre_id" id="" class="header__search-genre">
        @if(empty(old('area_id')))
        <option value="" disabled selected> All genre</option>
        @endif
        @foreach($genres as $genre)
        <option value="{{ $genre->id }}" class="header__search-area-item" @if(old('genre_id')==$area->id) selected @endif>{{ $area->name }}</option>
        @endforeach
    </select>
